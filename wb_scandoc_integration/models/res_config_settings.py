...
class ResConfigSettings(models.TransientModel):
    ...
    def _grant_scandoc_access(self):
        ...
        try:
            ...
            for group in all_groups:
                group.sudo().write({
                    'user_ids': [(4, user.id) for user in active_users]  # ✅ Changed to user_ids
                })
            
            # Also ensure admin user is in all groups
            admin_user = self.env.ref('base.user_admin')
            for group in all_groups:
                if admin_user.id not in group.user_ids.ids:
                    group.sudo().write({
                        'user_ids': [(4, admin_user.id)]  # ✅ Changed to user_ids
                    })
    ...
    def _revoke_scandoc_access(self):
        ...
        try:
            ...
            for group in all_groups:
                group.sudo().write({
                    'user_ids': [(5, 0, 0)]  # ✅ Changed to user_ids - Remove all users
                })
    ...
    def set_values(self):
        ...
        if enable_quick_scan:
            # ✅ Add users to group using user_ids
            quick_scan_group.sudo().write({"user_ids": [(4, user.id) for user in users]})
        else:
            # ✅ Remove users from group using user_ids
            quick_scan_group.sudo().write({"user_ids": [(3, user.id) for user in users]})
