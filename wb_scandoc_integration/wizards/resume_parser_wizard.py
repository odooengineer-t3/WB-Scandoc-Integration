...
class ResumeParserWizard(models.TransientModel):
    ...
    def process_resume(self):
        ...
        if skills:
            ...
            application_vals["applicant_skill_ids"] = self._process_skills(skills)

        # Work Experience
        work_experience = extracted.get("work_experience", [])
        if work_experience:
            total_months = 0
            work_experience_lines = [(5, 0, 0)]
        ...
        if summary.get("summary_text"):
            application_vals["description"] = summary.get("summary_text")
        ...
        if personal_info.get("full_name"):
            ...
            # Odoo 17 candidate name
            application_vals["partner_name"] = name
    ...
    def process_purchase_receipt(self):
        ...
        if not picking_type:
            raise UserError(_("No Incoming Picking Type found for this company. Please configure one."))

        # ---------------------------------------------------------
        # 🔧 FIX: Robust Location Resolution
        # Ensure we have valid source and destination locations
        # ---------------------------------------------------------
        location_id = picking_type.default_location_src_id.id
        if not location_id:
             # Fallback: Find a supplier location
             partner_loc = self.env['stock.location'].search([
                 ('usage', '=', 'supplier'),
                 ('company_id', 'in', [self.env.company.id, False])
             ], limit=1)
             if partner_loc:
                 location_id = partner_loc.id
        
        location_dest_id = picking_type.default_location_dest_id.id
        if not location_dest_id:
             # Fallback: Find a standard internal stock location
        ...
        # Final validation to prevent "Mandatory field not set" error
        if not location_id:
            raise UserError(_("Could not determine a valid Source Location (Supplier). Please check your Inventory settings (Picking Type or Locations)."))
        if not location_dest_id:
            raise UserError(_("Could not determine a valid Destination Location. Please check your Inventory settings."))
